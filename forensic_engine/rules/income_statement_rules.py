...
def check_income_statement_rules(row: Dict[str, Any]) -> Tuple[int, List[str]]:
    ...
    if net_income > 5e7:
        if cfo <= 0:
            score += 25
            msg = f"【净现比恶性断裂】净利润盈利 (${net_income/1e6:.1f}M) 但经营活动现金流为净流出 (${cfo/1e6:.1f}M)" if zh else f"[Malignant Cash Decoupling] Positive Net Income (${net_income/1e6:.1f}M) accompanied by negative CFO (${cfo/1e6:.1f}M)"
            warnings.append(msg)
        elif (cfo / net_income) < 0.30:
            score += 15
            msg = f"【现金流造血孱弱】净现比仅为 {cfo/net_income:.2f} (远低于0.5健康警戒线)，存在严重纸面富贵" if zh else f"[Weak Cash Flow Generation] CFO to Net Income ratio is {cfo/net_income:.2f} (critically below 0.50 healthy threshold)"
            warnings.append(msg)

    # 规则 2.2: 扣非/主营经营利润巨亏，靠非经常性损益/投资收益粉饰
    if net_income > 0 and op_inc < 0 and net_income > 2e7:
        score += 20
        msg = f"【主营造血枯竭】主营营业利润亏损 (${op_inc/1e6:.1f}M) 但净利润依靠非经常性损益/公允价值掩护为正 (${net_income/1e6:.1f}M)" if zh else f"[Core Operating Exhaustion] Operating loss (${op_inc/1e6:.1f}M) masked by non-operating windfalls to sustain net profit (${net_income/1e6:.1f}M)"
        warnings.append(msg)
    ...
    if sales > 1e9 and net_income > 0:
        ...
        if net_margin < 0.005:  # 净利率 < 0.5%
            score += 15
            msg = f"【总额法流水刷单嫌疑】营收超十亿美元 (${sales/1e6:.0f}M) 但净利率仅为 {net_margin*100:.2f}%，典型通道贸易虚增流水" if zh else f"[Pass-Through Revenue Inflation] High revenue (${sales/1e6:.0f}M) but negligible net margin ({net_margin*100:.2f}%), signaling commodity pass-through volume churning"
            warnings.append(msg)
    ...
    if sales > 0 and contract_assets > 0:
        ...
        if ca_ratio > 0.50 and contract_assets > 5e7:
            score += 15
            msg = f"【合同资产畸高】合同资产占收入比重达 {ca_ratio*100:.1f}% (${contract_assets/1e6:.1f}M)，警惕提前确认收入与后续大额冲减" if zh else f"[Excessive Contract Assets] Contract assets represent {ca_ratio*100:.1f}% of revenue (${contract_assets/1e6:.1f}M), premature revenue recognition risk"
            warnings.append(msg)
        ...
        if net_income < prev_net_income:  # 业绩已进入滑坡通道
            ...
            if payout_ratio > 0.50:
                ...
                msg = (
                    ...
                    if zh else
                    f"[Surge Payout Ahead of Downturn] Payouts reached ${total_payout/1e6:.1f}M ({payout_ratio*100:.1f}% of net income) despite earnings declining, warning for insider liquidity extraction"
                )
                warnings.append(msg)
        ...
        if q1_3_ni > 1e7 and q4_ni < -3e7:
            ...
            if loss_ratio > 0.50:
                ...
                msg = (
                    ...
                    if zh else
                    f"[Q4 Big-Bath Bathing] First three quarters profitable (${q1_3_ni/1e6:.1f}M), followed by abrupt massive Q4 loss (${abs(q4_ni)/1e6:.1f}M, wiping out {loss_ratio*100:.1f}% of 9-month income)"
                )
                warnings.append(msg)
